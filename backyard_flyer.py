...
class BackyardFlyer(Drone):
    ...
    def local_position_callback(self):
        ...
        if self.flight_state == States.TAKEOFF:
            z_dist_from_target = abs(self.global_position[2] - self.target_position[2])
            if z_dist_from_target < 1.0:
                self.all_waypoints = self.calculate_box()
                self.waypoint_transition()
        elif self.flight_state == States.WAYPOINT:
            dist_from_target = np.linalg.norm(self.target_position[0:2] - self.local_position[0:2])
    ...
    def velocity_callback(self):
        ...
        if self.flight_state == States.LANDING:
            z_dist_from_home = self.global_position[2] - self.global_home[2]
            if z_dist_from_home < 0.1:
    ...
    def calculate_box(self):
        ...
        """
        waypoints = [[8.0, 0.0, 4.0], [8.0, 8.0, 4.0], [0.0, 8.0, 4.0], [0.0, 0.0, 4.0]]
        return waypoints
